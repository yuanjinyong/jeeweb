    ...
    methods: {
      _loadEntity (cb) {
        this.$http.get(this.options.context.url + '/' + this.options.params.f_id).then((response) => {
          ...
          sql += '\n-- comment: 字典 ' + entity.f_code + ' ' + entity.f_name
          sql += '\n-- ' + entity.f_code + ' ' + entity.f_name + ' BEGIN ************************'
          sql += '\nDELETE FROM `t_sys_dict_item` WHERE f_dict_code = \'' + entity.f_code + '\';'
          sql += '\nDELETE FROM `t_sys_dict` WHERE f_code = \'' + entity.f_code + '\';'
          sql += '\n'
          sql += '\nINSERT INTO `t_sys_dict`(`f_code`,`f_name`,`f_db_name`,`f_table_name`,`f_tenant_column`,`f_code_column`,`f_name_column`,`f_order_column`,`f_where_clause`,`f_is_preset`,`f_remark`) VALUES '
          ...
          if (entity.itemList && entity.itemList.length > 0) {
            ...
            entity.itemList.forEach((item) => {
              sql += '\nINSERT INTO `t_sys_dict_item`(`f_tenant_id`,`f_dict_code`,`f_item_order`,`f_item_code`,`f_item_text`,`f_is_preset`) VALUES '
          ...
          sql += '\n-- rollback DELETE FROM `t_sys_dict_item` WHERE f_dict_code = \'' + entity.f_code + '\';'
          sql += '\n-- rollback DELETE FROM `t_sys_dict` WHERE f_code = \'' + entity.f_code + '\';'
