...
import com.jeeweb.platform.security.web.handler.AuthorizationSuccessHandler;
...
public class RestSecurityFilter extends AbstractSecurityInterceptor implements Filter {
    ...
    private List<AuthorizationSuccessHandler> authorizationSuccessHandlers = new ArrayList<>();
    ...
    public void doFilter(ServletRequest request, ServletResponse response, FilterChain chain)
        ...
        if (RequestUtil.isMatcher((HttpServletRequest) request, ignoringMatchers)) {
    ...
    private void doFilterInternal(FilterInvocation fi) throws IOException, ServletException {
        // 如果为不登录也可访问的URL
        if (RequestUtil.isMatcher(fi.getRequest(), permitAllMatchers)) {
            fi.getChain().doFilter(fi.getRequest(), fi.getResponse());
            return;
        }
        ...
        // 如果为登录即可访问的URL
        if (RequestUtil.isMatcher(fi.getRequest(), authenticatedMatchers)) {
            onAuthenticationSuccess(fi);
            fi.getChain().doFilter(fi.getRequest(), fi.getResponse());
            return;
        }
    ...
    private void onAuthenticationSuccess(FilterInvocation fi) {
        for (AuthorizationSuccessHandler authorizationSuccessHandler : authorizationSuccessHandlers) {
            authorizationSuccessHandler.onAuthorizationSuccess(fi);
        }
    }
     ...
     * @param handler
     */
    public void addAuthorizationSuccessHandler(AuthorizationSuccessHandler handler) {
        authorizationSuccessHandlers.add(handler);
    }

    /**
     * @param handlers
     */
    public void addAuthorizationSuccessHandlers(List<AuthorizationSuccessHandler> handlers) {
        authorizationSuccessHandlers.addAll(handlers);
    }
